...
import { ArrowLeft, CheckCircle2, Clock3, LogOut, Mail, RefreshCw, Save, ShieldCheck, Upload } from 'lucide-react'
...
const MAX_UPLOAD_BYTES = 3 * 1024 * 1024 // keep in sync with api/upload.js
...
function ImageUploadField({ label, value, onChange, adminKey }) {
  ...
  async function handleFile(e) {
    ...
    if (file.size > MAX_UPLOAD_BYTES) { setError('Image is too large. Please use a file under 3MB.'); return }
    ...
    try {
      ...
      const result = await adminFetch('/api/upload', adminKey, {
        method: 'POST',
  ...
  return (
    <div>
      <p className="text-sm text-porcelain/80">{label}</p>
      <div className="mt-2 flex items-center gap-4">
        <div className="h-20 w-28 shrink-0 overflow-hidden rounded-lg border border-brass/10 bg-ink">
          {value
            ? <SafeImage src={value} alt="" className="h-full w-full object-cover" />
            : <div className="flex h-full items-center justify-center text-center text-xs text-fog">No image</div>}
        </div>
        <div className="flex-1 space-y-2">
          <label htmlFor={inputId} className={`inline-flex cursor-pointer items-center gap-2 rounded-lg border border-brass/30 px-4 py-2 text-sm text-brass hover:bg-brass/10 ${uploading ? 'opacity-50' : ''}`}>
            <Upload className="h-4 w-4" />
            {uploading ? 'Uploading…' : value ? 'Replace image' : 'Upload image'}
          </label>
          <input id={inputId} type="file" accept="image/*" className="hidden" onChange={handleFile} disabled={uploading} />
          <input className={fieldClass} placeholder="or paste an image URL" value={value || ''} onChange={e => onChange(e.target.value)} />
        </div>
      </div>
...
export default function Admin() {
  ...
  if (!authenticated) return (
    <main className="min-h-dvh bg-ink text-porcelain px-6 py-12 md:py-20"><div className="mx-auto max-w-md">
  ...
  return <main className="min-h-dvh bg-ink text-porcelain">
    ...
    <div className="mx-auto max-w-7xl px-6 py-8 md:px-10">
      ...
      {tab === 'content' ? <ContentEditor content={content} setContent={setContent} saveContent={saveContent} saving={saving} adminKey={key} /> : tab === 'bookings' ? <Bookings bookings={bookings} updateStatus={updateStatus} /> : <Contacts contacts={contacts} />}
    </div>
  </main>
}

function ContentEditor({ content, setContent, saveContent, saving, adminKey }) {
  const c = content
